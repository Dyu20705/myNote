...
| Integrated-base CI | PR #96 run #331 was green at head `f84a688`; historical evidence only |
...
| Named measure | Credit | Evidence, result, and exact command |
| --- | ---: | --- |
| Typography/measure | 5/5 | [`accepted Figma aliases and intentional font stacks are exposed by CSS`](../tests/e2e/visual-system.spec.mjs) and the three generated `readable editor and long mixed content remain bounded at …` cases; integrated-base GREEN via `npm run test:e2e`. |
...
| Named measure | Credit | Evidence, result, and exact command |
| --- | ---: | --- |
| Three viewports | 5/5 | The generated `board overlay and note transient surfaces stay contained at 1024x768`, `…1280x720`, and `…1440x900` cases in [`desktop-resilience.spec.mjs`](../tests/e2e/desktop-resilience.spec.mjs); integrated-base GREEN via `npm run test:e2e`. |
...
| Visual/resize | `readable editor and long mixed content remain bounded at …`; `live desktop resize preserves query draft overlay and logical focus`; `npm run test:e2e` | Integrated-base GREEN | Native zoom excluded |
...
| Focused #73 cross-package | `release shell exposes only owner-backed navigation and bounded commands`; `workspace transitions preserve ordinary context and keyboard return`; four generated viewport cases; `saved-grid drawing stays local and outside canonical note content`; `npx --no-install playwright test tests/e2e/ux-release-gate.spec.mjs --project=chromium` | ENVIRONMENT BLOCKED | No product RED observed; no score credit |
...
| --- | --- | --- | --- |
| `1024×768` | Generated containment cases in `editor-shell.spec.mjs`, `visual-system.spec.mjs`, and `desktop-resilience.spec.mjs`; `npm run test:e2e` | Integrated-base GREEN | Supported Chromium CSS viewport |
| `1280×720` | Same generated suites and command; `npm run test:e2e` | Integrated-base GREEN | Supported Chromium CSS viewport |
| `1440×900` | Same generated suites and command; `npm run test:e2e` | Integrated-base GREEN | Supported Chromium CSS viewport |
...
The focused resource command is `npx --no-install playwright test tests/e2e/kanji-resource.spec.mjs --project=chromium`. Its exact test title is [`bounded drawing evidence validates maximum V2 shape, reloads note context, and renders 64 previews`](../tests/e2e/kanji-resource.spec.mjs). The checked-in 2026-08-10 Windows Chromium record reports:

| Operation | Evidence | Result | Notes/owner |
| --- | --- | --- | --- |
| Validate and codec-serialize maximum V2 shape | Fixture: 32 strokes, one 256-point stroke, 4,096 total points; 1 exact combined-operation warm-up; 5 samples. Durations: `105.4`, `104.6`, `87.9`, `93.3`, `105.8 ms`. | PASS against each-sample `<1,000 ms`; byte bounds also passed | Windows focused run; conservative tripwire, not cross-machine guarantee |
| Load/reload note context | Fixture: 65 valid minimal V2 entries; 2 synchronization calls. Durations: `4.9`, `5.0 ms`. | PASS against each-call `<2,000 ms` | Same Windows run; do not compare with unlike machines |
| Render projection window | Same 65-entry note; exactly 1 preview initially and 64 after disclosure; expanded render `175 ms`. | PASS against `<5,000 ms`; older-entry disclosure remained available | Same Windows run |
| Repeated open/close | `repeated open and close retains one dialog, stylesheet, command, and bounded desktop layout`; focused resource command above | Integrated-base GREEN | 17-cycle checked-in loop; one retained owner |
| Pointer fallback cleanup | `capture fallback finishes outside releases and leaves no temporary document listeners`; focused resource command above | Integrated-base GREEN | Temporary document listeners removed |
| Preview observer cleanup | `preview layout observer ownership stays bounded across hidden synchronization and teardown`; focused resource command above | Integrated-base GREEN | Observer ownership remains bounded |
| Command/history bounds | `registry rejects duplicate IDs and enforces a literal command bound`; `history enforces literal operation and snapshot bounds`; `npm run test:unit` | Integrated-base GREEN | Command, operation, snapshot, and detailed-patch retention are literal bounds |
...
| P2 — General performance targets lack representative benchmarks | Performance budget and inventory | OPEN | Startup/search/autosave/memory values remain targets; performance owner must add comparable benchmark evidence before promoting them to guarantees. |
