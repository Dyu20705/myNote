...
The E2E test uses only existing UI behavior. With `#contentInput` focused, it invokes the existing `Control+Enter` autosave flush path, then waits for `#saveState` to equal `Saved locally`; that state appears only after `putNoteToDb` resolves (and the subsequent search refresh completes) in the existing save path. The journey is create → edit synthetic content → assert `#saveState` is `Unsaved changes` → focus the editor and press `Control+Enter` → wait for `#saveState` to equal `Saved locally` using a locator assertion → reload → assert the edited title and synthetic content through the normal UI. Real Chromium execution showed that clicking `#saveButton` while `#contentInput` is focused first fires the existing blur handler, which calls `autosave.flush()` before the button click handler; a click-based test could therefore pass with a broken button handler and cannot attribute durability to that click. The focused-editor keyboard action avoids that blur masking. The interim assertion prevents the initial bootstrap value from creating a false green. It uses no fixed sleep, direct IndexedDB read, test-only production hook, or new Save control. A new Playwright context proves reload persistence only; it does not claim crash or safe-mode recovery.
...
| E2E durability | Focused-editor `Control+Enter` plus `#saveState` `Saved locally`, followed by reload UI assertions |
